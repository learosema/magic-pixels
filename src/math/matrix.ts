...
export class Matrix {
   ...
   */
  static fromVector(value: Vector): Matrix {
    if (value instanceof Vector) {
      return new Matrix(value.toArray(), { numRows: value.dim, numCols: 1 });
    }
   ...
   */
  static fromVectors(vectors: Vector[]): Matrix {
    ...
    return new Matrix(matrix, { numRows: dimension, numCols: vectors.length });
  }
   ...
   */
  mul(param: Matrix | number | Vector): Matrix | Vector {
    ...
    if (param instanceof Vector) {
      ...
      const m = this.mul(Matrix.fromVector(param)) as Matrix;
    ...
    if (param instanceof Matrix) {
      ...
      return new Matrix(multipliedValues, { numRows, numCols });
    }
    throw Error('ArgumentError');
  }

  div(param: number | Matrix): Matrix {
    if (typeof param === 'number') {
      const multipliedValues: number[] = this.values.map(
        (value) => value / param
      );
    ...
    if (param instanceof Matrix) {
      const mat = param as Matrix;
      const inversed: Matrix | Vector | number = mat.inverse();
      return this.mul(inversed) as Matrix;
    }
    throw Error('ArgumentError');
  }
