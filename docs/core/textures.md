...
A {@link Texture} is an image and four sampling settings. Like every scene
object it is plain data; the renderer uploads it to the GPU the first time
it is used as a uniform.

## Image sources

`image` can be an `HTMLImageElement`, `HTMLVideoElement`,
`HTMLCanvasElement` or `ImageData`. `Texture.fromImageUrl(url)` loads an
image with `crossOrigin = 'anonymous'` (so images from other origins work
when the server allows it) and resolves with a texture once decoded.
...
files store their first row at the _top_. The renderer does not flip on
upload, so an image appears upside down when UVs assume the top-left origin
of a picture. Flip the V coordinate in the shader (`1.0 - vUv.y`) or author
the UVs the other way round.

## Upload and lifetime

The first time a texture is bound the renderer creates a GL texture,
uploads the image as 8-bit RGBA, sets the four sampling parameters and, if
needed, generates mipmaps. The GL texture is cached in a map keyed by the
{@link Texture} object; the same texture in ten materials is uploaded once.
